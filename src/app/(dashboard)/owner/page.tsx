    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4">
        <Card className="animate-fade-in stagger-1">
          <CardHeader className="flex flex-row items-center justify-between pb-2 gap-1">
            <CardTitle className="text-xs sm:text-sm font-medium text-slate-500 dark:text-slate-400 leading-tight">Total Driver</CardTitle>
            <Users className="w-4 h-4 text-slate-400 shrink-0" />
          </CardHeader>
          <CardContent>
            <div className="text-xl sm:text-2xl font-bold text-slate-800 dark:text-white">{stats.totalDrivers}</div>
            <p className="text-[10px] sm:text-xs text-slate-400">Driver aktif</p>
          </CardContent>
        </Card>

        <Card className="animate-fade-in stagger-2">
          <CardHeader className="flex flex-row items-center justify-between pb-2 gap-1">
            <CardTitle className="text-xs sm:text-sm font-medium text-slate-500 dark:text-slate-400 leading-tight">Laporan Hari Ini</CardTitle>
            <FileText className="w-4 h-4 text-slate-400 shrink-0" />
          </CardHeader>
          <CardContent>
            <div className="text-xl sm:text-2xl font-bold text-slate-800 dark:text-white">{stats.todayReports}</div>
            <p className="text-[10px] sm:text-xs text-slate-400">Laporan masuk</p>
          </CardContent>
        </Card>

        <Card className="animate-fade-in stagger-3">
          <CardHeader className="flex flex-row items-center justify-between pb-2 gap-1">
            <CardTitle className="text-xs sm:text-sm font-medium text-slate-500 dark:text-slate-400 leading-tight">Setoran Pending</CardTitle>
            <DollarSign className="w-4 h-4 text-slate-400 shrink-0" />
          </CardHeader>
          <CardContent>
            <div className="text-xl sm:text-2xl font-bold text-slate-800 dark:text-white">{stats.pendingDeposits}</div>
            <p className="text-[10px] sm:text-xs text-slate-400">Menunggu</p>
          </CardContent>
        </Card>

        <Card className="animate-fade-in stagger-4">
          <CardHeader className="flex flex-row items-center justify-between pb-2 gap-1">
            <CardTitle className="text-[10px] sm:text-sm font-medium text-slate-500 dark:text-slate-400 leading-tight">Setoran Bulanan</CardTitle>
            <div className="flex items-center gap-1 sm:gap-2 shrink-0">
              <span className="text-[10px] sm:text-xs font-medium text-teal-600 dark:text-teal-400 bg-teal-50 dark:bg-teal-900/20 px-1.5 sm:px-2 py-0.5 rounded-full">{currentMonthLabel}</span>
              <TrendingUp className="w-4 h-4 text-slate-400 shrink-0" />
            </div>
          </CardHeader>
          <CardContent>
            <div className="text-lg sm:text-2xl font-bold text-slate-800 dark:text-white truncate">{formatCurrency(stats.monthlyDeposits)}</div>
            ...
            <p className="text-[10px] sm:text-xs text-slate-400 mt-1 truncate">{progressPercentage}% dari {formatCurrency(totalTarget)}</p>
          </CardContent>
        </Card>
      </div>
      ...
      {driverDeposits.length > 0 && driverDeposits.some(dd => dd.monthlyDeposit > 0) ? (
        <Card className="animate-fade-in">
          <CardHeader>
            <CardTitle className="flex flex-wrap items-center gap-1.5 sm:gap-2 text-xs sm:text-base">
              <Target className="w-4 h-4 sm:w-5 sm:h-5 text-teal-600 shrink-0" />
              Monitoring Setoran Per Driver
              <span className="text-[10px] sm:text-xs font-medium text-teal-600 dark:text-teal-400 bg-teal-50 dark:bg-teal-900/20 px-1.5 sm:px-2 py-0.5 rounded-full">{currentMonthLabel}</span>
              <Badge variant="default" className="ml-auto text-[10px] sm:text-xs shrink-0">
                Target {formatCurrency(MONTHLY_TARGET)}/bln
              </Badge>
            </CardTitle>
          </CardHeader>
          <CardContent>
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-1.5 sm:gap-2 mb-1">
                        <p className="text-xs sm:text-sm font-semibold text-slate-800 dark:text-white truncate max-w-[120px] sm:max-w-none">{dd.driverName}</p>
                        <span className="text-[10px] sm:text-xs text-slate-400 shrink-0">({dd.vehiclePlate})</span>
                      </div>
        ...
        <Card className="animate-fade-in">
          ...
          <CardContent>
            <div className="text-center py-8 text-slate-400">
              ...
              <p className="text-sm">Belum ada setoran bulan ini</p>
            </div>
          </CardContent>
        </Card>
      )}
        ...
        <CardHeader>
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
            <CardTitle className="flex items-center gap-1.5 sm:gap-2">
              <Truck className="w-4 h-4 sm:w-5 sm:h-5 text-teal-600 shrink-0" />
              <span className="text-xs sm:text-base leading-tight">Pantau Pendapatan Driver</span>
            </CardTitle>
      ...
      <Card className="animate-fade-in">
        ...
        <CardContent>
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 sm:gap-3">
            <a href="/owner/drivers" className="p-2.5 sm:p-4 border border-slate-200 dark:border-slate-700 rounded-lg hover:bg-slate-50 dark:hover:bg-slate-800 transition-all text-center">
              <Users className="w-4 h-4 sm:w-5 sm:h-5 text-teal-600 dark:text-teal-400 mx-auto mb-1 sm:mb-2" />
              <p className="text-[10px] sm:text-sm font-medium dark:text-white leading-tight">Kelola Driver</p>
            </a>
            <a href="/owner/reports" className="p-2.5 sm:p-4 border border-slate-200 dark:border-slate-700 rounded-lg hover:bg-slate-50 dark:hover:bg-slate-800 transition-all text-center">
              <FileText className="w-4 h-4 sm:w-5 sm:h-5 text-teal-600 dark:text-teal-400 mx-auto mb-1 sm:mb-2" />
              <p className="text-[10px] sm:text-sm font-medium dark:text-white leading-tight">Lihat Laporan</p>
            </a>
            <a href="/owner/deposits" className="p-2.5 sm:p-4 border border-slate-200 dark:border-slate-700 rounded-lg hover:bg-slate-50 dark:hover:bg-slate-800 transition-all text-center">
              <DollarSign className="w-4 h-4 sm:w-5 sm:h-5 text-teal-600 dark:text-teal-400 mx-auto mb-1 sm:mb-2" />
              <p className="text-[10px] sm:text-sm font-medium dark:text-white leading-tight">Setoran</p>
            </a>
            <a href="/owner/analytics" className="p-2.5 sm:p-4 border border-slate-200 dark:border-slate-700 rounded-lg hover:bg-slate-50 dark:hover:bg-slate-800 transition-all text-center">
              <Target className="w-4 h-4 sm:w-5 sm:h-5 text-teal-600 dark:text-teal-400 mx-auto mb-1 sm:mb-2" />
              <p className="text-[10px] sm:text-sm font-medium dark:text-white leading-tight">Analitik</p>
            </a>
          </div>
        </CardContent>
      </Card>
